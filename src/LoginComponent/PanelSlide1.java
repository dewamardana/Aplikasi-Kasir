...
import formPetugas.petugasMenu;
...
public class PanelSlide1 extends javax.swing.JLayeredPane {
    ...
    public PanelSlide1() {
        ...
        loading.addEventContinue(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent ae) {
                petugasMenu main = new petugasMenu();
    ...
    private void login(String userName, String password) {
        th = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    Thread.sleep(2000);
                    String sql = "select id_petugas, username, `Profile` from `tb_datapetugas` where BINARY(username)=? and BINARY(`password`)=? limit 1";
                    PreparedStatement p = DatabaseConnection.getInstance().getConnection().prepareStatement(sql);
                    p.setString(1, userName);
                    p.setString(2, password);
                    ResultSet r = p.executeQuery();
                    if (r.first()) {
                        int id = r.getInt("id_petugas");
